 ...
 * @see LICENSE.txt
 *
 * This file: Chinese (traditional) language data for the front-end (last modified: 2017.03.04).
 */
...
$phpMussel['lang']['config_attack_specific_block_control_characters'] = '受阻任何文件包含任何控製字符嗎（以外換行符）？ (<code>[\x00-\x08\x0b\x0c\x0e\x1f\x7f]</code>) 如果您只上傳純文本，您可以激活這個指令以提供某些另外保護在您的系統。然而，如果您上傳任何事以外純文本，激活這個可能結果在假陽性。 False = 不受阻【默認】； True = 受阻。';
...
$phpMussel['lang']['config_attack_specific_general_commands'] = '搜索文件內容為通用命令例如<code>eval()`和`exec()</code>？ False = 不搜索【默認】； True = 搜索。 關閉這個指令如果您打算上傳任何的下列在您的系統或CMS通過您的瀏覽器：PHP，JavaScript，HTML，python，perl文件和等等。激活這個指令如果您不有任何另外保護在您的系統和不打算上傳這些文件。如果您使用另外安全在連詞的phpMussel例如ZB Block，沒有任何需要激活這個指令，因為最的什麼phpMussel將尋找（在上下文這個指令）是重複的保護已提供。';
$phpMussel['lang']['config_attack_specific_scannable_threshold'] = '可選限或門檻為原始數據長度phpMussel是允許為閱讀和掃描（如果有任何引人注目性能問題當掃描）。值是一個整數代表文件大小在KB。 默認 = 32768 （32MB）。 零或空值將關閉門檻。按說，這個數值應不會少於平均文件大小的文件上傳您想和期待收到您的服務器或網站，應不會多於<code>filesize_limit`指令，和應不會多於大致五分之一的總允許內存分配獲授PHP通過`php.ini</code>配置文件。這個指令存在為嘗試防止phpMussel從用的太多內存（這個將防止它從能夠順利掃描文件以上的一個特別文件大小）。';
$phpMussel['lang']['config_compatibility_ignore_upload_errors'] = '這個指令按說應會關閉除非它是需要為對功能的phpMussel在您的具體系統。按說，當是關閉，當phpMussel檢測存在元素在<code>$_FILES`數組，它將嘗試引發一個掃描的文件代表通過那些元素，和，如果他們是空或空白，phpMussel將回報一個錯誤信息。這個是正確行為為phpMussel。然而，為某些CMS，空元素在`$_FILES</code>可以發生因之的自然的行為的那些CMS，或錯誤可能會報告當沒有任何，在這種情況，正常行為為phpMussel將會使乾擾為正常行為的那些CMS。如果這樣的一個情況發生為您，激活這個指令將指示phpMussel不嘗試引發掃描為這樣的空元素，忽略他們當發現和不回報任何關聯錯誤信息，從而允許延續的頁面請求。 False = 不忽略； True = 忽略。';
...
$phpMussel['lang']['config_general_forbid_on_block'] = 'phpMussel應該發送<code>403`頭隨著文件上傳受阻信息，或堅持標準</code>200 OK`？ False = 發送<code>200</code>； True = 發送<code>403</code>【標準】。';
...
$phpMussel['lang']['config_general_quarantine_key'] = 'phpMussel可以檢疫壞文件上傳在隔離在phpMussel的安全／保險庫｢Vault｣，如果這個是某物您想。 普通用戶的phpMussel簡單地想保護他們的網站或宿主環境無任何興趣在深深分析任何嘗試文件上傳應該離開這個功能關閉，但任何用戶有興趣在更深分析的嘗試文件上傳為目的惡意軟件研究或為類似這樣事情應該激活這個功能。 檢疫的嘗試文件上傳可以有時還助攻在調試假陽性，如果這個是某物經常發生為您。 以關閉檢疫功能，簡單地離開<code>quarantine_key</code>指令空白，或抹去內容的這個指令如果它不已空白。以激活隔離功能，輸入一些值在這個指令。 <code>quarantine_key`是一個重要安全功能的隔離功能需要以預防檢疫功能從成為利用通過潛在攻擊者和以預防任何潛在執行的數據存儲在檢疫。`quarantine_key`應該被處理在同樣方法作為您的密碼：更長是更好，和緊緊保護它。為獲得最佳效果，在結合使用`delete_on_sight</code>。';
...
$phpMussel['lang']['config_signatures_fail_extensions_silently'] = 'phpMussel應該報告當擴展是失踪嗎？ 如果<code>fail_extensions_silently`是關閉，失踪擴展將會報告當掃描，和如果`fail_extensions_silently</code>是激活，失踪擴展將會忽略，有掃描報告為那些文件哪裡沒有任何問題。 關閉的這個指令可能的可以增加您的安全，但可能還導致一個增加的假陽性。 False = 是關閉； True = 是激活【默認】。';
$phpMussel['lang']['config_signatures_fail_silently'] = 'phpMussel應該報告當簽名文件是失踪或損壞嗎？ 如果<code>fail_silently`是關閉，失踪和損壞文件將會報告當掃描，和如果`fail_silently</code>是激活，失踪和損壞文件將會忽略，有掃描報告為那些文件哪裡沒有問題。 這個應該按說被留下除非您遇到失敗或有其他類似問題。 False = 是關閉； True = 是激活【默認】。';
...
$phpMussel['lang']['config_virustotal_vt_quota_rate'] = '根據｢Virus Total API｣閱讀材料，它是限於最大的<code>4`請求的任何類型在任何`1</code>分鐘大致時間。如果您經營一個“honeyclient”， 蜜罐或任何其他自動化將會提供資源為VirusTotal和不只取回報告您是有權一個更高請求率配額。 作為標準，phpMussel將嚴格的堅持這些限制，但因為可能性的這些率配額被增加，這些二指令是提供為您指示phpMussel為什麼限它應堅持。 除非您是指示這樣做，它是不推薦為您增加這些數值，但，如果您遇到問題相關的到達您的率配額，減少這些數值可能有時幫助您解析這些問題。 您的率限是決定作為<code>vt_quota_rate`請求的任何類型在任何`vt_quota_time</code>分鐘大致時間。 vt_quota_rate:';
