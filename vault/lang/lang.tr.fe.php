 ...
 * @see LICENSE.txt
 *
 * This file: Turkish language data for the front-end (last modified: 2017.06.22).
 * (NOT COMPLETE)
 */
...
$phpMussel['lang']['config_files_block_encrypted_archives'] = 'Detect and block encrypted archives? Because phpMussel isn\'t able to scan the contents of encrypted archives, it\'s possible that archive encryption may be employed by an attacker as a means of attempting to bypass phpMussel, anti-virus scanners and other such protections. Instructing phpMussel to block any archives that it discovers to be encrypted could potentially help reduce any risk associated with these such possibilities. False = No; True = Yes [Varsayılan].';
$phpMussel['lang']['config_files_check_archives'] = 'Attempt to check the contents of archives? False = Don\'t check; True = Check [Varsayılan]. Currently, the only archive and compression formats supported are BZ/BZIP2, GZ/GZIP, LZF, PHAR, TAR and ZIP (archive and compression formats RAR, CAB, 7z and etcetera not currently supported). This is not foolproof! While I highly recommend keeping this turned on, I can\'t guarantee it\'ll always find everything. Also be aware that archive checking currently is not recursive for PHARs or ZIPs.';
$phpMussel['lang']['config_files_filesize_archives'] = 'Carry over filesize blacklisting/whitelisting to the contents of archives? False = No (just greylist everything); True = Yes [Varsayılan].';
$phpMussel['lang']['config_files_filesize_limit'] = 'Filesize limit in KB. 65536 = 64MB [Varsayılan]; 0 = No limit (always greylisted). Any (positive) numeric value accepted. This can be useful when your PHP configuration limits the amount of memory a process can hold or if your PHP configuration limits filesize of uploads.';
$phpMussel['lang']['config_files_filesize_response'] = 'What to do with files that exceed the filesize limit (if one exists). False = Whitelist; True = Blacklist [Varsayılan].';
$phpMussel['lang']['config_files_filetype_archives'] = 'Carry over filetype blacklisting/whitelisting to the contents of archives? False = No (just greylist everything) [Varsayılan]; True = Yes.';
...
$phpMussel['lang']['config_general_enable_plugins'] = 'Enable support for phpMussel plugins? False = No; True = Yes [Varsayılan].';
$phpMussel['lang']['config_general_forbid_on_block'] = 'Should phpMussel send 403 headers with the file upload blocked message, or stick with the usual 200 OK? False = No (200); True = Yes (403) [Varsayılan].';
...
$phpMussel['lang']['config_general_honeypot_mode'] = 'When honeypot mode is enabled, phpMussel will attempt to quarantine every single file upload that it encounters, regardless of whether or not the file being uploaded matches any included signatures, and no actual scanning or analysis of those attempted file uploads will actually occur. This functionality should be useful for those that wish to use phpMussel for the purposes of virus/malware research, but it\'s neither recommended to enable this functionality if the intended use of phpMussel by the user is for actual file upload scanning, nor recommended to use the honeypot functionality for purposes other than honeypotting. Varsayılan olarak, this option is disabled. False = Disabled [Varsayılan]; True = Enabled.';
...
$phpMussel['lang']['config_signatures_detect_adware'] = 'Should phpMussel parse signatures for detecting adware? False = No; True = Yes [Varsayılan].';
$phpMussel['lang']['config_signatures_detect_deface'] = 'Should phpMussel parse signatures for detecting defacements and defacers? False = No; True = Yes [Varsayılan].';
$phpMussel['lang']['config_signatures_detect_joke_hoax'] = 'Should phpMussel parse signatures for detecting joke/hoax malware/viruses? False = No; True = Yes [Varsayılan].';
$phpMussel['lang']['config_signatures_detect_packer_packed'] = 'Should phpMussel parse signatures for detecting packers and packed data? False = No; True = Yes [Varsayılan].';
$phpMussel['lang']['config_signatures_detect_pua_pup'] = 'Should phpMussel parse signatures for detecting PUAs/PUPs? False = No; True = Yes [Varsayılan].';
$phpMussel['lang']['config_signatures_detect_shell'] = 'Should phpMussel parse signatures for detecting shell scripts? False = No; True = Yes [Varsayılan].';
$phpMussel['lang']['config_signatures_fail_extensions_silently'] = 'Should phpMussel report when extensions are missing? If <code>fail_extensions_silently</code> is disabled, missing extensions will be reported on scanning, and if <code>fail_extensions_silently</code> is enabled, missing extensions will be ignored, with scanning reporting for those files that there aren\'t any problems. Disabling this directive may potentially increase your security, but may also lead to an increase of false positives. False = Disabled; True = Enabled [Varsayılan].';
$phpMussel['lang']['config_signatures_fail_silently'] = 'Should phpMussel report when signatures files are missing or corrupted? If <code>fail_silently</code> is disabled, missing and corrupted files will be reported on scanning, and if <code>fail_silently</code> is enabled, missing and corrupted files will be ignored, with scanning reporting for those files that there aren\'t any problems. This should generally be left alone unless you\'re experiencing crashes or similar problems. False = Disabled; True = Enabled [Varsayılan].';
...
$phpMussel['lang']['config_urlscanner_maximum_api_lookups_response'] = 'İzin verilen maksimum API arama sayısı aşıldığında ne yapılması gerekir? False = Hiçbir şey yapma (ışleme devam et) [Varsayılan]; True = Dosyayı engelleyin.';
