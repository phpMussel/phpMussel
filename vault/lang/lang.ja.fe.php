 ...
 * @see LICENSE.txt
 *
 * This file: Japanese language data for the front-end (last modified: 2017.06.12).
 */
...
$phpMussel['lang']['config_files_filesize_limit'] = 'ファイルサイズ上限の単位はKBです。 65536 = 64MB （Default/デフォルト）； 0 = リミットしません（上限なし、常にグレイリスト化）、正の数値であれば何でも構いません。 ＰＨＰの設定でメモリーに制限があったり、アップロードファイルサイズの上限が設定されている場合に有効的です。';
...
$phpMussel['lang']['config_files_max_uploads'] = '一度にスキャンできるアップロードファイル数の上限で、これを超えるとスキャンを中断し、ユーザーにその旨を知らせ、論理攻撃からの保護として機能します。 システムやCMSがDDoS攻撃にあい、phpMusselがオーバーロードしてＰＨＰプロセスに支障をきたすことがないようにするためです。 推奨数は１０ですが、ハードウェアのスピードによっては、これ以上/以下がよいということもあるでしょう。この数は、アーカイブのコンテンツは含まないことを覚えておいて下さい。';
