 ...
 * @see LICENSE.txt
 *
 * This file: Russian language data for the front-end (last modified: 2017.02.07).
 */
...
$phpMussel['lang']['config_general_cleanup'] = 'После исполнения стирает переменные величины данного руководства и очищает кэш-память. Если Вы не хотите пользоваться руководством после того, как проверили загрузку программы, то установите опцию `true`, чтобы минимизировать загрузку памяти. Если Вы будете использовать руководство и для других целей, то установите опцию `false`, дабы избежать ненужного многократного считывания данных в памяти. Как правило, устанавливается опция `true`, но потом Вы сможете пользоваться руководством только для контроля файлов. Не воздействует на CLI-модус.';
...
$phpMussel['lang']['config_general_quarantine_max_filesize'] = 'Максимально допустимый объём файлов, которые могут быть изолированы в карантине. В карантинном регистре НЕ будет сохраняться файлы, объём которых больше указанного значения. Эта команда важна тем, что затрудняет потенциальному агрессору наводнить нежелательными данными ваш карантинный регистр, а также кэш-память вашего хост-сервера. Значение измеряется килобайтами. Стандарт = 2048 = 2048KB = 2MB.';
$phpMussel['lang']['config_general_quarantine_max_usage'] = 'Максимально допустимая загрузка карантина. Когда общий объём файлов на карантине достигает этого значения, то начинают удаляться старые файлы пока не будет достигнуто необходимое значение. Эта команда важна тем, что затрудняет потенциальному агрессору наводнить нежелательными данными ваш карантинный регистр, а также кэш-память вашего хост-сервера. Значение измеряется килобайтами. Стандарт = 65536 = 65536KB = 64MB.';
...
$phpMussel['lang']['config_general_timeOffset'] = 'Если ваш сервер времени не соответствует вашему местному времени, Вы можете указать смещение здесь для корректировки информации о дате/времени генерируется от phpMussel в соответствии с вашими потребностями. Обычно рекомендуется вместо того, чтобы настроить директиву о зоне времени в файле `php.ini`, но иногда (например, при работе с общими хостинг-провайдеров, которые ограничены) это не всегда возможно, и так, эта опция представлена здесь. Смещение описывается как минут.';
...
$phpMussel['lang']['config_signatures_MD5'] = 'Скан с MD5-сигнатуры? Если да, то указать имена файлов сигнатур для использования, через запятую.';
$phpMussel['lang']['config_signatures_PE_Extended'] = 'Скан PE-файлов (портативный исполняемый / portable executable; EXE, DLL и др.) с PE расширенного сигнатуры? Если да, то указать имена файлов сигнатур для использования, через запятую.';
$phpMussel['lang']['config_signatures_PE_Sectional'] = 'Скан PE-файлов (портативный исполняемый / portable executable; EXE, DLL и др.) с PE-секционными сигнатуры? Если да, то указать имена файлов сигнатур для использования, через запятую.';
...
$phpMussel['lang']['config_urlscanner_urlscanner'] = 'В phpMussel есть URL сканер, способен обнаружить вредоносные URL-адреса изнутри данных или файлов отсканированных. Для активирования URL сканер задайте директива `urlscanner` значение в `true`; Для деактивизация задайте в `false`.';
