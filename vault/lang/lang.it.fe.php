 ...
 * @see LICENSE.txt
 *
 * This file: Italian language data for the front-end (last modified: 2017.02.07).
 */
...
$phpMussel['lang']['config_attack_specific_decode_threshold'] = 'Opzionale limitazione o soglia per la lunghezza dei grezzi dati dove decodificare comandi dovrebbe essere rilevati (nel caso in cui vi siano notevoli problemi di prestazioni durante la scansione). Il valore è un integer che rappresenta la dimensione dei file in KB. Predefinito = 512 (512KB). Un zero o un nullo valore disabilita la soglia (rimuovere tale limitazione basata sulla dimensione dei file).';
$phpMussel['lang']['config_attack_specific_general_commands'] = 'Cercare contenuti dei file per generali comandi quali `eval()`, `exec()` e `include`? False = No (no verifica) [Predefinito]; True = Sì (fare verifica). Disattivare questa opzione se si intende caricare qualsiasi delle seguenti al vostra sistema o CMS tramite il browser: PHP, JavaScript, HTML, python, perl file e eccetera. Attivare questa opzione se non avete qualsiasi aggiuntivi protezioni sul vostro sistema e non intendono caricare tali file. Se si utilizza qualsiasi aggiuntivi protezione in collaborazione con phpMussel come ZB Block, vi è non necessità di attivare questa opzione, perché la maggior parte di ciò che phpMussel sarà cercare (nel contesto di questa opzione) sono duplicazioni di protezioni che sono già forniti.';
$phpMussel['lang']['config_attack_specific_scannable_threshold'] = 'Opzionale limitazione o soglia per la lunghezza dei grezzi dati dove phpMussel è permesso di leggere e scansione (nel caso in cui vi siano notevoli problemi di prestazioni durante la scansione). Il valore è un integer che rappresenta la dimensione dei file in KB. Predefinito = 32768 (32MB). Un zero o un nullo valore disabilita la soglia. In generale, questo valore non dovrebbe essere meno quella media dimensione dei file che si desidera e si aspettano di ricevere al vostro server o al vostro web sito, non dovrebbe essere più di la filesize_limit direttiva, e non dovrebbe essere più di circa un quinto del totale ammissibile allocazione della memoria concesso al PHP tramite il file di configurazione `php.ini`. Questa direttiva esiste per tenta di evitare avendo phpMussel utilizzare troppa memoria (di cui sarebbe impedirebbe di essere capace di completare la file scansione correttamente per i file piú d\'una certa dimensione).';
...
$phpMussel['lang']['config_signatures_MD5'] = 'Verificare contro MD5 firme durante la scansione? In tal caso, specificare i nomi dei file di firme da utilizzare, delimitati da virgole.';
$phpMussel['lang']['config_signatures_PE_Extended'] = 'Verificare PE (Portatile Eseguibile) file (EXE, DLL, ecc) contro PE esteso firme durante la scansione? In tal caso, specificare i nomi dei file di firme da utilizzare, delimitati da virgole.';
$phpMussel['lang']['config_signatures_PE_Sectional'] = 'Verificare PE (Portatile Eseguibile) file (EXE, DLL, ecc) contro PE Sezionale firme durante la scansione? In tal caso, specificare i nomi dei file di firme da utilizzare, delimitati da virgole.';
...
$phpMussel['lang']['config_urlscanner_urlscanner'] = 'Costruito in phpMussel è un URL scanner, in grado di rilevare URL malevoli all\'interno di dati ei file scansionati. Per abilitare l\'URL scanner, imposta la `urlscanner` direttiva su true; Per disabilitarlo, imposta questa direttiva su false.';
...
$phpMussel['lang']['tip_see_the_documentation'] = 'Vedere la <a href="https://github.com/Maikuolan/phpMussel/blob/master/_docs/readme.it.md#SECTION7">documentazione</a> per informazioni sulle varie direttive di configurazione ed i loro scopi.';
