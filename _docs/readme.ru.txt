 ...
 5) Скрепите phpMussel с Вашей системой или с системой управления содержимым
    ...
    обозначается в регистре как `/includes`, `/assets` или `/functions`, и
    часто называется "init.php", "common_functions.php", "functions.php". Вы
 ...
 6) На этом инсталляционный процесс заканчивается. Теперь Вам необходимо
    ...
    функционировании программы появится сообщение от phpMussel, что загрузка
    (Upload) была успешно блокирована. Если такое сообщение не появилось, то
    программа функционирует неправильно. Воспользуйтесь другими расширенными
    функциями или всевозможными сканированными видами этой программы. Вы должны
    тестировать до тех пор, пока не убедитесь в нормальном функционировании
    программы.
 ...
 файлы, папки или архивы, указанные Вами неявно. Для этого убедитесь, что
 ...
 - Задается вектор для атак. Независимо от того, пользуетесь Вы phpMussel или
   нет, на Ваш сервер будут посылаться слепые команды (Probing). (Поэтому есть
   как сторонники, так и противники использования команд браузера). С одной
   стороны, не получив нужного результата, благодаря работе phpMussel, вы
   станете неинтересными для нападающих, с другой стороны, если появится
   непредвиденный или неизвестный для действующей версии phpMussel, или её
   будущим версиям, эксплоит, который предложит вектор атак, способных принести
   слепым командам положительный результат, он может воодушевить нападающих на
   новые атаки Вашей системы.
 ...
 * Название файла может варьироваться, в зависимости от конфигурации в
 ...
 - Генеральная конфигурация от phpMussel.
    ...
    - После исполнения стирает переменные величины данного руководства и
      ...
      того, как проверили загрузку программы, то установите опцию «да» ("yes"),
      чтобы минимизировать загрузку памяти. Если вы будете использовать
      руководство и для других целей, то установите опцию «нет» ("no"), дабы
      избежать ненужного многократного считывания данных в памяти. Как правило,
      устанавливается опция «да» ("yes"), но потом Вы сможете пользоваться
      руководством только для контроля файлов.
      * Не воздействует на CLI-модус.
    "scan_log"
    ...
    - Эта опция позволяет в течение сканирования немедленно удалять файлы при
      ...
      0 – После проверки файл останется нетронутым [Стандарт];
      1 – После проверки инфицированный файл будет немедленно удалён.
    "lang"
 ...
 - Конфигурация подписями.
   ...
   - Скан PE-файлов (Portable Executable, EXE, DLL и др.) с PE-секционными
     подписями? 0 = Нет; 1 = Да [Стандарт].
     "pe_clamav"
     "pe_custom"
     "pe_mussel"
   - Скан PE-файлов (Portable Executable, EXE, DLL и др.) с PE
     расширенного опознавания подписями? 0 = Нет; 1 = Да [Стандарт].
     "pex_custom"
     "pex_mussel"
 ...
 - Файл обработки конфигурации.
   ...
   - Нужно ли узнавать и блокировать зашифрованные архивы? phpMussel не может
     ...
     риски, связанные с этой возможностью. 0 = Нет; 1 = Да [Стандарт].
 ...
 Последний раз эта информация обновлялась 28.05.2015 года и считается
 актуальной для всех опубликованных на данный момент phpMussel и обеих её
 дополнительных версий (v0.5-v0.6i).
