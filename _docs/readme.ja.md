...
"MD5" （ＭＤ５）
- スキャニング時にＭＤ５シグネチャに対してチェックしますか？ その場合は、使用するシグネチャファイルの名前をコンマで区切って指定します。

"PE_Sectional" （ＰＥ・セクショナル）
- スキャニング時にＰＥ（ポータブル・エグゼキュータブル）ファイル（ＥＸＥ、ＤＬＬ、等）をＰＥセクショナルシグネチャに対してチェックしますか？ その場合は、使用するシグネチャファイルの名前をコンマで区切って指定します。

"PE_Extended" （ＰＥ・イクステンディッド）
- スキャニング時にＰＥ（ポータブル・エグゼキュータブル）ファイル（ＥＸＥ、ＤＬＬ、等）をＰＥ拡張シグネチャに対してチェックしますか？ その場合は、使用するシグネチャファイルの名前をコンマで区切って指定します。

スキャニング時に一般シグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "general_clamav"
- "general_mussel"

スキャニング時に正規ASCIIシグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "ascii_clamav"
- "ascii_mussel"

スキャニング時に正規ＨＴＭＬシグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "html_clamav"
- "html_mussel"

スキャニング時にＰＥ（ポータブル・エグゼキュータブル）ファイル（ＥＸＥ、ＤＬＬ、等）をＰＥシグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "exe_clamav"
- "exe_mussel"

スキャニング時にELFファイルをＥＬＦシグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "elf_clamav"
- "elf_mussel"

スキャニング時にMach-Oファイル（OSX、など）をMach-Oシグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "macho_clamav"
- "macho_mussel"

スキャニング時にグラフィックファイルをグラフィックベースシグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "graphics_clamav"
- "graphics_mussel"

スキャニング時にOLEオブジェクトをＯＬＥシグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "ole_clamav"
- "ole_mussel"

スキャニング時にファイル名をファイル名ベースシグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "filenames_clamav"
- "filenames_mussel"

スキャンする際に、電子メールシグネチャを使用しますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "mail_clamav"
- "mail_mussel"
...
スキャニング時に複合拡張シグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "coex_clamav"
- "coex_mussel"

スキャニング時にＰＤＦシグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "pdf_clamav"
- "pdf_mussel"

ショックウェーブシグネチャに対してチェックしますか？ `false`（偽） = いいえ； `true`（真） = はい （Default/デフォルト）。
- "swf_clamav"
- "swf_mussel"
...
- `0`： phpMusselのシグネチャを使ってスキャンした結果、ヒューリスティックな重みがあると判断された場合にのみ、疑わしいファイルと結論付けられます。すなわちTotal APIは、phpMusselが危険性を察知はしたが完全にそうとは言い切れず、したがってブロックもせず、フラグを付けることもしなかった時のセカンドオピニオンです。
...
- phpMusselにはURLスキャナーがビルトインされていて、スキャンされたファイルやデータ内の悪質なURLを検出することができます。 URLスキャナーを有効にするには、使用するシグネチャファイルの名前をコンマで区切って指定します。
...
####*シグネチャ詳細*
phpMusselが使うシグネチャタイプの詳細です。
- "正規ASCIIシグネチャ"　スキャン対象ファイルがホワイトリスト化されていない場合、そのコンテンツに対して使用されます。
- "複合拡張シグネチャ"　ミックスしたシグネチャタイプのマッチングです。
- "ＥＬＦシグネチャ"　スキャン対象ファイルがホワイトリスト化されていない場合、そのコンテンツに対して使用され、 ELFフォーマットへの適合をみます。
- "ポータブル実行シグネチャ"　スキャン対象ファイルがホワイトリスト化されていない場合、そのコンテンツに対して使用され、PEフォーマットへの適合をみます。
- "ファイル名シグネチャ"　スキャン対象ファイルのファイル名に対して使用されます。
- "一般シグネチャ"　スキャン対象ファイルがホワイトリスト化されていない場合、そのコンテンツに対して使用されます。
- "グラフィックシグネチャ"　スキャン対象ファイルがホワイトリスト化されていない場合、そのコンテンツに対して使用され、既知のグラフィカルファイルフォーマットへの適合をみます。
- "一般コマンド" (hex_general_commands.csv)　スキャン対象ファイルがホワイトリスト化されていないファイルのコンテンツに対して使用されます。
- "正規ＨＴＭＬシグネチャ"　スキャン対象ファイルがホワイトリスト化されていないＨＴＭＬファイル場合、そのコンテンツに対して使用されます。
- "マッチＯシグネチャ" スキャン対象ファイルがホワイトリスト化されていない場合、そのファイルのコンテンツに対して使用され、マッチＯフォーマットへの適合をみます。
