...
7) À ce stade, vous avez fini! Cependant, vous devriez probablement tester ce pour s'assurer qu'il fonctionne correctement. Pour tester les protections, essayez de télécharger les tester fichiers inclus dans l'empaqueter sous `_testfiles` à votre website par votre habituelles navigateur basé méthodes de téléchargement. Si tout fonctionne correctement, un message devrait apparaître à partir de phpMussel confirmant que le téléchargement a été bloqué avec succès. Si rien ne s'affiche, quelque chose ne fonctionne pas correctement. Si vous utilisez d'avancées fonctions ou si vous utilisez l'autres types d'analyse possibles avec l'outil, je vous suggère de l'essayer avec ceux pour s'assurer qu'il fonctionne comme prévu, aussi.
...
2) Ouvrir `phpmussel.php`, chercher pour la ligne commençant par `$vault=`, et remplacer la string entre guillemets suivantes sur cette ligne avec le véritable exact emplacement du le répertoire `vault` de phpMussel. Vous aurez remarqué un tel dossier dans l'archive que vous avez téléchargé (sauf si vous sentez à nouveau codage de l'ensemble du script, vous aurez besoin à maintenir la même structure de fichiers et de répertoires comme il était dans l'origine archive). Ce `vault` répertoire devrait être d'un niveau au-dessus le répertoire que le fichier `phpmussel.php` existera po. Enregistrer le fichier, fermer.

4) (En option; Fortement recommandé pour l'avancés utilisateurs, mais pas recommandé pour les débutants ou pour les novices): Ouvrir `phpmussel.ini` (situé à l'intérieur de `vault`) - Ce fichier contient toutes les directives disponible pour phpMussel. Au-dessus de chaque option devrait être un bref commentaire décrivant ce qu'il fait et ce qu'il est pour. Réglez ces options comme bon vous semble, selon ce qui est approprié pour votre particulière configuration. Enregistrer le fichier, fermer.
...
phpMussel est prévu à être un script que fonctionnera correctement dès la boîte avec un minimum niveau des exigences de votre part: Une fois qu'il a été installé, au fond, il devrait simplement travailler.

Numérisation de fichiers téléchargé est automatisée et activée par défaut, donc rien n'est requise de votre part pour cette particulière fonction.

Cependant, vous êtes également capable de instruire phpMussel à rechercher des fichiers, répertoires ou archives que vous spécifiez implicitement. Pour ce faire, d'abord, vous devez assurer que l'appropriée configuration est réglée dans le `phpmussel.ini` fichier (cleanup doit être désactivé), et lorsque fini, dans un php fichier qui est lié à phpMussel, utiliser la fonction suivante dans votre code:

`phpMussel($quoi_a_recherche,$sortie_type,$sortie_platitude);`

Où:
- `$quoi_a_recherche` est une string ou un tableau, pointant à un cible fichier, un cible répertoire ou un tableau de cibles fichiers et/ou cibles répertoires.
- `$sortie_type` est un entier, indiquant le format dans lequel les résultats de l'analyse doivent être retour. Une valeur de 0 instruit que la fonction d'affichage des résultats comme un entier (un retourné résultat de -2 indique que corrompues données était détecté lors de l'analyse et donc l'analyse n'ont pas réussi à compléter, -1 indique que les extensions ou addons requis par PHP pour exécuter l'analyse sont manquaient et donc l'analyse n'ont pas réussi à compléter, 0 indique qu'il n'existe pas cible à analyser et donc il n'y avait rien à analyser, 1 indique que la cible était analysé avec succès et aucun problème n'été détectée, et 2 indique que la cible était analysé avec succès et problèmes ont été détectés). Une valeur de 1 indique à la fonction pour renvoyer les résultats sous forme de texte lisible par l'homme. Une valeur de 2 indique à la fonction pour renvoyer les résultats sous forme de texte lisible par l'homme et pour exporter les résultats à une globale variable. Cette variable est facultative, 0 par défaut.
- `$sortie_platitude` est un entier, indiquant si les résultats pourront être retournés comme un tableau ou pas. Normalement, si la cible de l'analyse contenue plusieurs articles (par exemple, si un répertoire ou un tableau) les résultats seront retournés dans un tableau (défaut valeur de 0). Une valeur de 1 instruit la fonction pour imploser tous tableaux avant l'entrée, résultant en une aplatie string contenant les résultats à être retourner. Cette variable est facultative, 0 par défaut.
...
Quelques raisons pour lesquelles vous -ne devriez pas- permettre à ces contrôles:
...
Fichier                                    | Description
-------------------------------------------|--------------------------------------
/phpmussel.php                             | Chargement fichier. Charge le principal script et etc. C'est ce que vous êtes censé être accrochage dans à (essentiel)!
...
/_testfiles/html_standard_testfile.txt     | Test fichier à test phpMussel normalisé HTML signatures.
...
/vault/lang/lang.en.inc                    | Linguistiques données: ENGLISH
/vault/lang/lang.es.inc                    | Linguistiques données: ESPAÑOL
/vault/lang/lang.fr.inc                    | Linguistiques données: FRANÇAIS
/vault/lang/lang.id.inc                    | Linguistiques données: BAHASA INDONESIA
/vault/lang/lang.it.inc                    | Linguistiques données: ITALIANO
/vault/lang/lang.ja.inc                    | Linguistiques données: 日本語
...
/vault/ascii_clamav_regex.cvd              | Fichier pour normalisé ASCII signatures.
/vault/ascii_clamav_regex.map              | Fichier pour normalisé ASCII signatures.
/vault/ascii_clamav_standard.cvd           | Fichier pour normalisé ASCII signatures.
/vault/ascii_clamav_standard.map           | Fichier pour normalisé ASCII signatures.
/vault/ascii_custom_regex.cvd              | Fichier pour normalisé ASCII signatures.
...
/vault/general_clamav_regex.cvd            | Fichier pour général signatures.
/vault/general_clamav_regex.map            | Fichier pour général signatures.
/vault/general_clamav_standard.cvd         | Fichier pour général signatures.
/vault/general_clamav_standard.map         | Fichier pour général signatures.
/vault/general_custom_regex.cvd            | Fichier pour général signatures.
/vault/general_custom_standard.cvd         | Fichier pour général signatures.
/vault/general_mussel_regex.cvd            | Fichier pour général signatures.
/vault/general_mussel_standard.cvd         | Fichier pour général signatures.
/vault/graphics_clamav_regex.cvd           | Fichier pour graphiques signatures.
/vault/graphics_clamav_regex.map           | Fichier pour graphiques signatures.
/vault/graphics_clamav_standard.cvd        | Fichier pour graphiques signatures.
/vault/graphics_clamav_standard.map        | Fichier pour graphiques signatures.
/vault/graphics_custom_regex.cvd           | Fichier pour graphiques signatures.
/vault/graphics_custom_standard.cvd        | Fichier pour graphiques signatures.
/vault/graphics_mussel_regex.cvd           | Fichier pour graphiques signatures.
/vault/graphics_mussel_standard.cvd        | Fichier pour graphiques signatures.
...
/vault/hex_general_commands.csv            | Hex-codé CSV de généraux commande détections optionnellement utilisés par phpMussel.
/vault/html_clamav_regex.cvd               | Fichier pour normalisé HTML signatures.
/vault/html_clamav_regex.map               | Fichier pour normalisé HTML signatures.
/vault/html_clamav_standard.cvd            | Fichier pour normalisé HTML signatures.
/vault/html_clamav_standard.map            | Fichier pour normalisé HTML signatures.
/vault/html_custom_regex.cvd               | Fichier pour normalisé HTML signatures.
/vault/html_custom_standard.cvd            | Fichier pour normalisé HTML signatures.
/vault/html_mussel_regex.cvd               | Fichier pour normalisé HTML signatures.
/vault/html_mussel_standard.cvd            | Fichier pour normalisé HTML signatures.
/vault/lang.inc                            | Linguistiques données.
/vault/macho_clamav_regex.cvd              | Fichier pour Mach-O signatures.
/vault/macho_clamav_regex.map              | Fichier pour Mach-O signatures.
/vault/macho_clamav_standard.cvd           | Fichier pour Mach-O signatures.
/vault/macho_clamav_standard.map           | Fichier pour Mach-O signatures.
/vault/macho_custom_regex.cvd              | Fichier pour Mach-O signatures.
/vault/macho_custom_standard.cvd           | Fichier pour Mach-O signatures.
/vault/macho_mussel_regex.cvd              | Fichier pour Mach-O signatures.
/vault/macho_mussel_standard.cvd           | Fichier pour Mach-O signatures.
/vault/mail_clamav_regex.cvd               | Fichier pour mail signatures.
/vault/mail_clamav_regex.map               | Fichier pour mail signatures.
/vault/mail_clamav_standard.cvd            | Fichier pour mail signatures.
/vault/mail_clamav_standard.map            | Fichier pour mail signatures.
/vault/mail_custom_regex.cvd               | Fichier pour mail signatures.
...
/vault/xmlxdp_clamav_regex.cvd             | Fichier pour XML/XDP morceaux signatures.
/vault/xmlxdp_clamav_regex.map             | Fichier pour XML/XDP morceaux signatures.
/vault/xmlxdp_clamav_standard.cvd          | Fichier pour XML/XDP morceaux signatures.
/vault/xmlxdp_clamav_standard.map          | Fichier pour XML/XDP morceaux signatures.
/vault/xmlxdp_custom_regex.cvd             | Fichier pour XML/XDP morceaux signatures.
/vault/xmlxdp_custom_standard.cvd          | Fichier pour XML/XDP morceaux signatures.
/vault/xmlxdp_mussel_regex.cvd             | Fichier pour XML/XDP morceaux signatures.
/vault/xmlxdp_mussel_standard.cvd          | Fichier pour XML/XDP morceaux signatures.
...
- N'a pas d'influence en CLI mode.
"scan_log"
- Nom du fichier à enregistrer tous les résultats d'analyse à. Spécifiez un nom de fichier, ou laisser vide à désactiver.
"scan_kills"
- Nom du fichier à enregistrer tous les résultats de bloqué ou tué téléchargements à. Spécifiez un nom de fichier, ou laisser vide à désactiver.
"ipaddr"
...
-- "sd_siglen_max"
"fail_silently"
- Devrait phpMussel signaler lorsque les signatures fichiers sont manquants ou endommagés? Si fail_silently est désactivé, manquants et corrompus fichiers seront signalé sur analyse, et if fail_silently est activé, manquants et corrompus fichiers seront ignorés, avec l'analyse signalés pour ceux fichiers qu'il n'y a pas de problèmes. Cela devrait généralement être laissé seul sauf si vous rencontrez accidents ou similaires problèmes. 0 = Désactivé, 1 = Activé [Défaut].
...
"filesize_response"
- Que faire avec des fichiers qui dépassent la taille de fichier limite (si existant). 0 - Énumérer Blanche, 1 - Énumérer Noire [Défaut].
...
"check_archives"
- Essayez vérifier le contenu des archives? 0 - Non (ne pas vérifier), 1 - Oui (vérifier) [Défaut].
...
"filesize_archives"
- Étendre taille du fichier liste noire/blanche paramètres à le contenu des archives? 0 - Non (énumérer grise tout), 1 - Oui [Défaut].
"filetype_archives"
- Étendre type de fichier liste noire/blanche paramètres à le contenu des archives? 0 - Non (énumérer grise tout), 1 - Oui [Défaut].
"max_recursion"
...
- "Portable Executable Sectional Signatures" (pe_*). Vérifié contre le taille et l'MD5 hash des sections de chaque PE fichier non listé blanche, ciblée pour l'analyse et identifié au PE format.
...
- "XML/XDP Morceaux Signatures" (xmlxdp_*). Vérifié contre de chaque XML/XDP morceaux trouvés dans tout fichier non listé blanche et ciblée pour l'analyse.
...
Dernière Réactualisé: 15 Juin 2015 (2015.06.15).
EOF