...
“enable_plugins”
- 啟用phpMussel插件支持嗎？ False = 不要啟用； True = 要啟用【標準】。

“forbid_on_block”
- phpMussel應該發送`403`頭隨著文件上傳受阻信息，或堅持標準`200 OK`？ False = 發送`200`； True = 發送`403`【標準】。
...
“disable_frontend”
- 關閉前端訪問嗎？前端訪問可以使phpMussel更易於管理，但也可能是潛在的安全風險。建議管理phpMussel通過後端只要有可能，但前端訪問提供當不可能。保持關閉除非您需要它。 False = 激活前端訪問； True = 關閉前端訪問【標準】。
...
“Filename”
- 檢查文件名針對基於文件名簽名當掃描嗎？ 如果是這樣，請指定要使用的簽名文件名稱，以逗號分隔。

“MD5”
- 檢查針對MD5簽名當掃描嗎？ 如果是這樣，請指定要使用的簽名文件名稱，以逗號分隔。

“PE_Sectional”
- 檢查移植可執行｢PE｣文件（EXE文件，DLL文件，等等）針對移植可執行｢PE｣部分簽名當掃描嗎？ 如果是這樣，請指定要使用的簽名文件名稱，以逗號分隔。

“PE_Extended”
- 檢查移植可執行｢PE｣文件（EXE文件，DLL文件，等等）針對移植可執行｢PE｣擴展簽名當掃描嗎？ 如果是這樣，請指定要使用的簽名文件名稱，以逗號分隔。
...
“urlscanner”
- 內phpMussel是一個URL掃描儀，能夠檢測惡意URL在任何數據或文件它掃描。 以激活URL掃描儀，請指定要使用的簽名文件名稱，以逗號分隔。
...
- “標準化ASCII簽名”。 匹配針對內容的所有非白名單文件目標為掃描。
- “複雜擴展簽名”。 雜簽名類型匹配。
- “ELF簽名”。 匹配針對內容的所有非白名單文件目標為掃描識別的ELF文件。
- “移植可執行｢PE｣簽名”。 匹配針對內容的所有非白名單掃描目標識別的移植可執行｢PE｣文件。
- “文件名簽名”。 匹配針對文件名的文件目標為掃描。
- “通用簽名”。 匹配針對內容的所有非白名單文件目標為掃描。
- “圖像簽名”。 匹配針對內容的所有非白名單文件目標為掃描識別的一個已知圖像文件格式。
- “通用命令”。 匹配針對內容的所有非白名單文件目標為掃描。
- “標準化HTML簽名”。 匹配針對內容的所有非白名單HTML文件目標為掃描。
- “Mach-O簽名”。 匹配針對內容的所有非白名單文件目標為掃描識別的Mach-O文件。
- “電子郵件簽名”。 匹配針對內容的所有非白名單EML文件目標為掃描。
- “MD5簽名”。 匹配針對MD5哈希的內容和文件大小的所有非白名單文件目標為掃描。
- “OLE簽名”。 匹配針對內容的所有非白名單OLE對象目標為掃描。
- “PDF簽名”。 匹配針對內容的所有非白名單PDF文件目標為掃描。
- “移植可執行｢PE｣部分簽名”。 匹配針對MD5哈希和大小的每移植可執行｢PE｣部分的所有非白名單文件目標為掃描識別的移植可執行｢PE｣文件。
- “移植可執行｢PE｣擴展簽名”。 匹配針對MD5哈希和大小的變量在所有非白名單文件目標為掃描識別的移植可執行｢PE｣文件。
- “SWF簽名”。 匹配針對內容的所有非白名單SWF文件目標為掃描。
- “白名單簽名”。 匹配針對MD5哈希的內容和文件大小的所有文件目標為掃描。識別文件將會免疫的成為匹配通過簽名類型提到從他們的白名單項。
（請注意任何的這些簽名可以很容易地關閉通過`config.ini`）。
