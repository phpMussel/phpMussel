...
scan_log_serialized
- Необходимый пароль: `logs_password`
- Следующие условия: Необходимо установить `scan_log_serialized`.
- Необходимые параметры: (нет)
- Дополнительные параметры: (нет)
- Пример: `?logspword=[logs_password]&phpmussel=scan_log_serialized`
- Цель: Выдача содержания файла `scan_log_serialized`.
...
/vault/controls.php | Обработчик контроль.
...
*Полезный совет: If you want, you can append date/time information to the names of your logfiles by including these in the name: `{yyyy}` for complete year, `{yy}` for abbreviated year, `{mm}` for month, `{dd}` for day, `{hh}` for hour. @TranslateMe@*
...
Последний раз эта информация обновлялась 2016.04.21 года и считается актуальной для всех опубликованных на данный момент phpMussel и обеих её дополнительных версий (v0.10.0-v1.0.0).
...
| Bkav                 |  Сообщает "VEXC640.Webshell", "VEXD737.Webshell", "VEX5824.Webshell"|
...
| Qihoo-360            |  Сообщает "Script/Trojan.Script.393" |
...
Последнее обновление: 3 Июнь 2016 (2016.06.03).
