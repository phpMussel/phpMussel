...
"Filename"
- Скан названий файлов с их подписями? Если да, то указать имена файлов сигнатур для использования, через запятую.

"MD5"
- Скан с MD5-сигнатуры? Если да, то указать имена файлов сигнатур для использования, через запятую.

"PE_Sectional"
- Скан PE-файлов (портативный исполняемый / portable executable; EXE, DLL и др.) с PE-секционными сигнатуры? Если да, то указать имена файлов сигнатур для использования, через запятую.

"PE_Extended"
- Скан PE-файлов (портативный исполняемый / portable executable; EXE, DLL и др.) с PE расширенного сигнатуры? Если да, то указать имена файлов сигнатур для использования, через запятую.
...
"urlscanner"
- В phpMussel есть URL сканер, способен обнаружить вредоносные URL-адреса изнутри данных или файлов отсканированных. Для активирования URL сканер, то указать имена файлов сигнатур для использования, через запятую.
...
- «Нормированные ASCII подписи». Проверяется содержание каждого файла, не внесённого в белый список и которые следует проверить.
- «Комплекса расширенного опознавания подписей». Подбираются смешанные типы подписями.
- «ELF подписи». Проверяется содержание каждого файла, не внесённого в белый список, а также его соответствие ELF-формату.
- «PE подписи». Проверяется содержание каждого файла, не внесённого в белый список, а также его соответствие PE-формату.
- «Подписи названий файлов». Проверяется содержание каждого файла, не внесённого в белый список и которые следует проверить.
- «Генеральные подписи». Проверяется содержание каждого файла, не внесённого в белый список и которые следует проверить.
- «Графические подписи». Проверяется содержание каждого файла, не внесённого в белый список и которые следует проверить, а также его соответствие известному графическому формату.
- «Общие команды». Проверяется содержание каждого файла, не внесённого в белый список и которые следует проверить.
- «Нормированные HTML-подписи». Проверяется содержание каждого HTML-файла, не внесённого в белый список и которые следует проверить.
- «Mach-O подписи». Проверяется содержание каждого файла, не внесённого в белый список и которые следует проверить, а также его соответствие Mach-O-формату.
- «Email подписи». Проверяется содержание каждого EML-файла, не внесённого в белый список и которые следует проверить.
- «MD5 подписи». Посредством MD5-Hash проверяется содержание и объём каждого файла, не внесенного в белый список и которые следует проверить.
- «OLE подписи». Проверяется содержание каждого объекта, не внесённого в белый список и которые следует проверить.
- «PDF подписи». Проверяется содержание каждого PDF-файла, не внесённого в белый список и которые следует проверить.
- «PE секционных подписей». Посредством значений и MD5-Hash PE-секций проверяется каждый файл, не внесённый в белый список и которые следует проверить, а также его соответствие PE-формату.
- «PE расширенного подписей». Посредством значений и MD5-Hash и объём переменных в каждого файла, не внесённый в белый список и которые следует проверить, а также его соответствие PE-формату.
- «SWF подписи». Проверяется содержание каждого Shockwave-файла, не внесённого в белый список.
- «Подписи белого списка». Посредством MD5-Hash проверяется содержание и объём каждого файла которые следует проверить. Соответствующие файлы иммунитет к типу подписей в белый список записи.
...
Последнее обновление: 14 Февраль 2017 (2017.02.14).
